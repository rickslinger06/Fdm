...
//		pet2.getOwners().remove(1); // removing owner from collections
//		em = emf.createEntityManager();
//		PetOwner foundOwner = em.find(PetOwner.class, owner2.getId());
//		em.getTransaction().begin();
//		em.merge(pet);
...
//		Helper.close(em);
		...
		/*
		 * Cannot delete Owning when they share inverse with other owning instances
		 */
		em = emf.createEntityManager();
		Pet foundPet = em.find(Pet.class, pet.getId());
		em.getTransaction().begin();
		foundPet.getOwners().remove(0);
		foundPet.getOwners().remove(0);
		em.remove(foundPet);
		em.getTransaction().commit();
		Helper.close(em);
